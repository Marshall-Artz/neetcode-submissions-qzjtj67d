class Solution:
    def asteroidCollision(self, asteroids: List[int]) -> List[int]:
        ...
        for ast in asteroids: # Loop through asteroids (left -> right)
            if (not stack or # if stack emtpy
                stack[-1] > 0 and ast > 0 or # or both moving right
                stack[-1] < 0 and ast < 0 or # or both moving left
                stack[-1] < 0 and ast > 0):  # or left moving left and right moving right
                stack.append(ast)
            else:
